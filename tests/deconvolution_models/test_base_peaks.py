...
from raman_fitting.deconvolution_models.base_peak import FieldsCoordinator, FieldsCoordinatorWarning

#%%

def _error_message_contains(excinfo, testmsg: str, verbose: bool = False):
    _fltr_str = [i if not i in ['(',')'] else ' '
...
class TestBasePeak(unittest.TestCase):
    ...
    def test_empty_base_class_raises(self):
        ...
        with pytest.raises(AttributeError) as excinfo:
            eb.peak_model
        assert _error_message_contains(excinfo, 'type object empty no attribute _peak_model')

        #%%
    def test_empty_base_class_with_kwargs_raises(self):
        #%%

        class EmptyTestChild(metaclass=BasePeak, testkwarg2=2, testkwarg3=3, peak_type='Voigt', verbose=True):
            pass
        ...
        with pytest.raises(ValueError) as excinfo:
            eb.peak_type = 'emptytest'
        self.assertTrue(_error_message_contains(excinfo, 'value emptytest for peak_type not in'))

        with pytest.raises(ValueError) as excinfo:
            eb.peak_name = 10*'emptytest'
        self.assertTrue(_error_message_contains(excinfo, 'value for peak_name is too long 90'))

        with pytest.raises(AttributeError) as excinfo:
            eb.peak_model
        self.assertTrue(_error_message_contains(excinfo, 'type object empty no attribute _peak_model'))

        with pytest.raises(ValueError) as excinfo:
            eb.peak_type = 'VoigtLorentzian'
        self.assertTrue(_error_message_contains(excinfo, ''''Multiple options ['Lorentzian', 'Voigt'] for misspelled value "VoigtLorentzian"'''))
...
#%%
    def test_base_class_good_with_init_extra_tests(self):
        ...
        _def_param = td1.param_hints_constructor(td1.fco.register['init']['param_hints'])
        self.assertEqual(_def_param['amplitude'].value, 14)

        with pytest.raises(TypeError) as excinfo:
            _def_param = td1.param_hints_constructor('fail')
        self.assertTrue(_error_message_contains(excinfo, "input_param_hints should be of type dictionary not <class 'str'>"))
        ...
        self.assertTrue(_error_message_contains(excinfo, " Unable to create a Parameter from center and (1, 2, 3, 4):"))
...
class TestFieldsCoordinator(unittest.TestCase):
    ...
    #%%
        fco = FieldsCoordinator(fields = ['peak_name','peak_type','param_hints'],
        ...
        self.assertIn('Field peak_name has multiple sources' , ', '.join(map(str, cm.warning.args)))

        with self.assertWarns(UserWarning) as cm:
            fco.store('notinsources','peak_name','R4D4')
        self.assertIn('Store in notinsources at peak_name' , ', '.join(map(str, cm.warning.args)))

        fco.store('init','peak_type','Voirentzian')

        with self.assertWarns(UserWarning) as cm:
            fco.store('init','peak_type','Voirentzian')
        self.assertIn('Redefinition of peak_type in init ignored' , ', '.join(map(str, cm.warning.args)))
        ...
        _test_dict = {'peak_name': 'multi_store', 'peak_type': 'multi_test', 'param_hints': 2}
