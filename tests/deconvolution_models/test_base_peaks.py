...
class TestBasePeak(unittest.TestCase):
    ...
    def test_empty_base_class_with_kwargs_raises(self):
        ...
        assert _error_message_contains(excinfo, ''''Multiple options ['Lorentzian', 'Voigt'] for misspelled value "VoigtLorentzian"''')
...
#%%
    def test_base_class_good_with_init_extra_tests(self):
        ...
        assert str(td1.peak_model) == peakmod
        _class_str = f"TestD1peak, {peakmod}, center : 2600 < 2650 > 2750"
        assert str(td1) == _class_str 
        td1.peak_name = 'R2D2'
        assert td1.peak_model.prefix == 'R2D2_'
        ...
        assert _def_param['amplitude'].value == 14
        
        with pytest.raises(TypeError) as excinfo:
            _def_param = td1.param_hints_constructor('fail')
        assert _error_message_contains(excinfo, "input_param_hints should be of type dictionary not <class 'str'>")
        ...
        assert _error_message_contains(excinfo, " Unable to create a Parameter from center and (1, 2, 3, 4):")
