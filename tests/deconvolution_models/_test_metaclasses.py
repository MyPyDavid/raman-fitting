...
class Parent(type):
    ...
    def __new__(mcls, name, bases, cls_dict, **kwargs):
        print(
            f"Called __new ({mcls} ),name {name}, bases{bases}, cls_dict {cls_dict} kwargs {kwargs}"
        )
        ...
        for _field in cls_object._fields:
            if hasattr(cls_object, _field):
                ...
                print(
                    f"__new getattr {_field} inside from child {_child_aname }, {type(_child_aname )}"
                )
        ...
        _other_methods = [
            i for i in dir(mcls) if not i.startswith("_") and not i == "mro"
        ]
