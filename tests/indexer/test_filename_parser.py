...
class TestFilenameParser(unittest.TestCase):
    ...
    def test_PP_extra_from_parts(self):
        self.assertEqual('TEST', self.empty_PP._extra_sgrID_overwrite_from_parts('TEST'))

        for k, val in self.empty_PP._extra_sgrpID_name_mapper.items():
            emptymap_PP = PathParser(f'{k}/TEST.txt')
            self.assertEqual(val,
                         emptymap_PP._extra_sgrID_overwrite_from_parts('TEST',
                                    mapper=emptymap_PP._extra_sgrpID_name_mapper))
