...
class MainDelegator:
    ...
    def simple_process_sample_wrapper(self, _gen, **kwargs):
        for sID_args in _gen:
            logger.warning(
                f"{self._cqnm} simple process_sample_wrapper args:\n\t - {_gen}\n\t - {kwargs}"
            )
            exp_sample = None
            try:
                logger.warning(
    ...
    def process_sample(self, *args, **kwargs):
        ...
        logger.warning(
            f'{self._cqnm} process sample {", ".join(map(str,args))}. on models:\n{models}'
        )
...
def process_sample_wrapper(fn, *args, **kwargs):
    def wrapper(*args, **kwargs):
        logger.warning(
            f"process_sample_wrapper process_sample_wrapper args:\n\t - {fn}\n\t - {args}\n\t - {kwargs}"
        )
        exp_sample = None
        try:
