...
class Parser:
    ...
    def parse_from_txt(self, usecols=(0, 1), delimiter='\t', unpack=True, skiprows=0):
        ...
        while not ramanshift.any() and i < 2000:
            try:
                ramanshift, intensity = np.loadtxt(self.filepath, usecols=usecols, delimiter=delimiter, unpack=unpack, skiprows=i)
                ...
                self.info.update({'spectrum_length': len(ramanshift),
                                  '_parser_skiprows': i, '_parser_delimeter': delimiter})
            except ValueError:
                i += 1
