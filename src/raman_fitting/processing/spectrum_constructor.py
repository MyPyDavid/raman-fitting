...
class SpectrumDataCollection:
    ...
    def test_spectra_lengths(self):
        ...
        if len(set_lengths) == 1:
            ...
            self._spectra = [spec for spec in self._spectra
        ...
        for spec in self._spectra:
            ...
            if hasattr(spec, 'clean_data'):
                if not _cdks:
                    ...
                    _cdks = {
                        x for x in _cdks if x in set(spec.clean_data.keys())}

                if not _prep_data:
                    _prep_data = {
                        key: [(spec.SamplePos, val)] for key, val in spec.clean_data.items()}
        ...
        for wndwnm, data in self.prep_clean_data.items():
            ...
            for _pos, _sp in data:
                ...
                _dfspec = pd.DataFrame(
                    {'ramanshift': _sp.ramanshift,
                     _pos_lbl: _sp.intensity})
