...
class FieldsCoordinator:
    ...
    def _check_if_register_sufficient(self,_reg):
        
        _fvals = []
        for sk, sval in self.register.items():
            _sreg = {k: val for k,val in sval.items() if val}
            _fvals.append(_sreg)
    ...
    def store(self, source, field, val):
        if source in self.sources and field in self.fields and val:
            ...
            if not _fval:
                ...
                warn('Redefinition of same value ignored', FieldsCoordinatorWarning)
            elif _fval != val:
                _src[field] = val
                warn(f'Overwriting of same value collision! {_fval} is not {val}', FieldsCoordinatorWarning)
            else:
                warn('Store {source} {val} unexpected', FieldsCoordinatorWarning)
